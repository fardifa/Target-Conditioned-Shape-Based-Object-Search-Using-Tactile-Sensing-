...
MODEL_XML  = os.path.join(SCRIPT_DIR, "scene_cone.xml")
BODY_NAME  = "pyramid"

# --- Motion constants ---
BASE_X       = 0.085
CONTACT_X    = 0.058
FIX_Z        = 0.070          # maintain this height
FORWARD_SPD  = 0.40
BACK_SPD     = 0.36
PAUSE_T      = 0.10
ROT_T        = 0.30
NUDGE_X      = 0.0005         # small forward push to maintain contact
FORCE_THR    = 0.0005         # lower threshold for sensitive detection
DAMPING      = 0.96
...
def main():
    print("🚀 Starting tactile pyramid simulation (stable contact + 3D logging)")
    ...
    # --- Dataset setup ---
    save_dir = os.path.join(SCRIPT_DIR, "dataset", "cone")
    ...
    def tactile_force():
        if model.nsensor < 1:
            return 0.0
        # pad_force = 3D vector, pad_touch = scalar
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running():
            ...
            if phase == "forward":
                ...
                # Maintain gentle pressure on pad
                if f_val < FORCE_THR:
